...
function ResetPassword() {
    ...
    const handleResetPassword = async (values) => {
        setLoading(true)
        try {
            await axios.put(`${serverUrl}/api/auth/reset-password/${resetToken}`, { password: values.password })
            setSuccess(true)
            toast.success("Password reset successfully!")
            setTimeout(() => navigate('/login'), 3000)
        } catch (error) {
            const apiMessage = error.response?.data?.message || 'Reset failed'
        ...
        <div className='min-h-screen bg-gradient-to-br from-gray-900 via-purple-900 to-gray-900 flex flex-col items-center justify-center p-6 relative overflow-hidden'>
            ...
            <div className='w-full max-w-md relative z-10 animate-fade-in'>
                ...
                <div className='text-center mb-8'>
                    <h2 className='text-4xl font-bold text-white mb-2'>Admin Reset Password</h2>
                ...
                <div className='bg-white/10 dark:bg-gray-800/90 backdrop-blur-xl border border-white/20 dark:border-gray-700 rounded-2xl shadow-2xl p-8'>
                    {success ? (
                        <div className="text-center">
                            <p className="text-green-400 mb-6">Password reset successfully! Redirecting to login...</p>
                        </div>
                    ) : (
                            ...
                            <div>
                                <div className='relative'>
                                    ...
                                    <button
                                        ...
                                        className='absolute right-3 top-1/2 -translate-y-1/2 text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-gray-100 transition-colors'
                                    >
                                        {showConfirmPassword ? <TbEyeClosed className='w-5 h-5' /> : <BsEye className='w-5 h-5' />}
                                    </button>
                                </div>
                                {errors.confirmPassword && (
                                    <span className='block mt-2 text-sm text-red-400'>{errors.confirmPassword.message}</span>
                                )}
                            </div>
